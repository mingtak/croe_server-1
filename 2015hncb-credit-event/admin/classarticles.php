...
class CArticles extends CEngine
{
   ...
   $this->DBQuery("SELECT sets.description AS setdescription,filename,authorID FROM ".$this->table[2]." AS files LEFT JOIN ".$this->table[12]." AS sets ON files.filesetID=sets.ID WHERE sets.ID='".$filesetID."' ORDER BY files.ID");
   }
    ...
    * @access public
    */
   function GetFileSetDescription()
   {
   return $this->access["setdescription"];
   }
    ...
    * @access public
    */
   function GetArticleStats($articleID=0)
   {
   if (!$articleID) $articleID=$this->articleID;
   $this->DBQuery("SELECT views FROM ".$this->table[4]." WHERE articleID='".$articleID."'");
   $this->DBGetRow();
   echo $this->access["views"];
   }
    ...
    * @access private
    */
   function TestSyndication($articleID)
   {
   if (!SYNDICATION) return;
   $this->DBQuery("SELECT * FROM ".$this->table[0]." WHERE section='".$this->server."'");
   if (!$this->rowsnumber) exit;
   $this->DBGetRow();
   $topsectionID=$this->access["ID"];
