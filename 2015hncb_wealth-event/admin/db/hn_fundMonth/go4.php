...
fwrite($fp, mb_convert_encoding('"姓名","遮蔽姓名","身分證後六碼","生日","Email","遮蔽Email","電話"', 'big5', 'UTF-8')."\r\n\r\n");

foreach ($_prize as $code => $count)
{
	fwrite($fp, mb_convert_encoding('="'.$_title[$code].'"', 'big5', 'UTF-8')."\r\n");
	...
	while ($f = $DB->Arrays($r))
	{
		...
		fwrite($fp, mb_convert_encoding('="'. $f['2014hncb_app_winnerName'] .'",="'. $n .'",="'. $f['2014hncb_app_winnerID6'] .'",="'. $f['2014hncb_app_winnerBirthday'] .'",="'. $f['2014hncb_app_winnerEmail'] .'",="'. $e .'",="'. $f['2014hncb_app_winnerPhone'] .'"', 'big5', 'UTF-8')."\r\n");
	}
	fwrite($fp, "\r\n");
}
